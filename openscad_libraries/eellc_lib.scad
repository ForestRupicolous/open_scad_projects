...
/****Basic Options****/
//
typ_e = 0; //Element Types: typ_e, Input- [0,1, or 2]
...
s_e = 48; // element size (triangle or square side-length; hexagon flat-to-flat width/minor diameter)
/*******/
typ_h =2; //Hook Types: typ_h, Input- [0 or 1]; Description-[square, rounded]
...
s_h = 2; // hook size (width/diameter of arcing/squared hook profile-tip)
/*******/
n_elem = 1; // "x" (column) array size
m_elem = 2; // "y" (row)    array size
/*******/
e_fill = 1; //Element Fill Type: e_fill, [-1,0, or 1]
...
fileName = "../tea_bot_holder.stl"; // file name
...
module combine_hook_element() {
    if (typ_h == 0 || typ_h ==2) place_hook(); // square hook
    if (typ_h == 1) { // rounded hook
    difference() {
        place_hook();
        place_hook_neg();
...
module removal_shape() { //material removal to insert hooks
    ...
    if ((typ_h == 0 || typ_h==2) && (typ_e == 2 || typ_e == 0)) { //hexagonal/square filled removal (Square Hooks)
        translate( [ 0, s_e / 2 , 0] ) {
                scale( [s_h, s_h, s_h ] ) {
                    translate( [-2 + (1 - k_h), -1.5 - (1-k_h), 0 ] ) {
                    square( [4 , 1 + k_h]);
...
module make_hook() {
     ...
     */
     ...
     if (typ_h == 2)  //triangle dovetail like hooks
     {
        translate ([-2 * s_h,- 2.5 * s_h, 0]) {
            scale([s_h, s_h, s_h]) {
            hookPoints = [
